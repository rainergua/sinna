...
const comboCasosSinPostEgreso = async (req, res) => {
    ...
    const query = {
        text: `select * from sinna_modefa.f_combo_casos_egresados_sin_post_egreso($1)`,
        values:[id]
    };
    await con
        .query(query)
...
const gestionPostEgreso = async (req, res) => {
    ...
    const query = {
        text: `call sinna_modefa.p_gestion_post_egreso($1) `,
        values:[v_json]
    };
    await con
        .query(query)
...
const listaPostEgreso = async (req, res) => {
    const id = req.params.id
    const query = {
        text: `select * from sinna_modefa.f_listar_post_egresos($1) `,
        values:[id]
    };
    await con
        .query(query)
...
const listaSeguimientosPostEgreso = async (req, res) => {
    const id = req.params.id
    const query = {
        text: `select * from sinna_modefa.f_listar_seguimientos_post_egreso($1) `,
        values:[id]
    };
    await con
        .query(query)
...
const gestionSeguimientoPostEgreso = async (req, res) => {
    ...
    const v_json = req.body;
    const query = {
        text: `call sinna_modefa.p_gestion_seguimiento_post_egreso($1) `,
        values:[v_json]
    };
    await con
        .query(query)
