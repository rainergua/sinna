...
const gestionRestitucion = async (req, res) => {
    req.body.ci_usuario = req.user.ci;
    const v_json = req.body;
    console.log('Consulta', req.body);
    const query = {
        text: `call sinna_modefa.p_gestion_restitucion($1) `,
        values:[v_json]
    };
...
const listaRestitucion = async (req, res) => {
    const id = req.params.id
    const tipo_doc = req.params.tipo
    const query = {
        text: `select * from sinna_modefa.f_mostrar_restitucion($1,$2) `,
        values:[id, tipo_doc]
    };
    await con
        .query(query)
